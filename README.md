...
![Схема основных сервисов](./docs/osn7.png)
...
|:----------------------------| :--- | :--- |
| **Основной**                | Центральный оркестратор (ОТС). Хранит бизнес-логику и управляет статусами. | При оформлении метит каждую позицию `warehouse_id`, определяя источник отгрузки. |
...
###  Структура логистических данных
...
*   **Гео-позиция (`location`)**: Служит единым реестром городов (`city`), местоположения груза.
...
## Склады 
*   **тип хранения** документоориентированная (Так как будет удобно хранить иерархии в виде структуры, и очень быстро воспроизводимую, при появлении больше складов.)
*   **гео распределенная** для максимальной быстрой доставки
### Сценарий    
* Делаем запрос по складам и выбираем от удаленности склада все позиции. Так как мы знаем что на 1 складе 10 штук мы забираем все 10, со второго 5 штук, с третьего 5 штук. Риск есть если например связь упала и у нас старые данные в сервисе заказа, либо мы вообще не можем получить, то мы принимаем заказ, и при появлении связи подтверждаем его.
...
| :--- |:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Управление доступом** | Реализация гибридной авторизации: поддержка собственной базы данных пользователей и интеграция с внешними сервисами (OAuth/Social Login). В таблице упращенный вариант |
...
| **Логистический модуль** | Расчет стоимости, сроков и отслеживания отправлений.                                                                                                                   |
...
| :--- |:----------------------------------------------------------------------------|
| **Payment Gateways** | Банк или провайдер для безопасной обработки платежей.                       |
